...
import de.berlios.jhelpdesk.web.charts.TicketStatsByPriorityDatasetProducer;

@Controller
public class TicketsByPriorityViewController {

    private static Log log = LogFactory.getLog(TicketsByPriorityViewController.class);

    @Autowired
    private StatsPriorityPAO statsPAO;

    @SuppressWarnings("unchecked")
    @RequestMapping("/stats/tickets/priority.html")
    public String handleRequest(ModelMap mav) {
        ...
        mav.addAttribute("currentWeek", new TicketStatsByPriorityDatasetProducer(currentWeekMap));
        mav.addAttribute("currentMonth", new TicketStatsByPriorityDatasetProducer(currentMonthMap));
        mav.addAttribute("threeMonth", new TicketStatsByPriorityDatasetProducer(threeMonthsMap));
        mav.addAttribute("allTime", new TicketStatsByPriorityDatasetProducer(allMap));

        return "stats/ticketsByPriority";
    }
}