			...
			<form action="<c:url value="/newBug.html"/>" method="post" enctype="multipart/form-data">
				<table id="table2" class="summary standardtable" cellspacing="0">
					...
					<tr>
						...
						<td class="lastcol"><c:out value="${hdbug.notifier.firstName}"/></td>
					</tr>
					<tr>
						<td class="desc">Nazwisko</td>
						<td class="lastcol"><c:out value="${hdbug.notifier.lastName}"/></td>
					</tr>
					<tr>
						<td class="desc">Telefon</td>
						<td class="lastcol"><c:out value="${hdbug.notifier.phone}"/></td>
					</tr>
					<tr>
						<td class="desc">E-mail</td>
						<td class="lastcol"><c:out value="${hdbug.notifier.email}"/></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="lastcol"><c:out value="${hdbug.subject}"/></td>
					</tr>
					<tr>
						<td class="desc">Opis</td>
						<td class="lastcol"><c:out value="${hdbug.description}"/></td>
					</tr>
					<c:if test="${not empty hdbug.stepByStep}">
					<tr>
						<td class="desc">Opis krok po kroku</td>
						<td class="lastcol"><c:out value="${hdbug.stepByStep}"/></td>
					</tr>
					</c:if>
					<c:if test="${not empty hdbug.addFilesList}">
					<tr>
						...
						<td class="lastcol">
							<table class="files standardtable" cellspacing="0">
								...
								<c:forEach var="file" items="${hdbug.addFilesList}" varStatus="status">
								<tr>
